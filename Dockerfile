FROM openjdk:8 as stream-app-build
...
COPY --from=stream-app-build /build/target/words-count-app.jar /app/

WORKDIR /app

EXPOSE 8080

ENTRYPOINT [ "scala", "words-count-app.jar" ]